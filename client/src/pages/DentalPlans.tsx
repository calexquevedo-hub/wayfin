...
import { Smile, Plus, Trash, X, Copy, CircleAlert, CircleCheck } from 'lucide-react';
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
const DentalPlans = () => {
    ...
    const fetchPlans = async () => {
        try {
            const { data } = await api.get('/dental-plans');
            setPlans(data);
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingPlan) {
                await api.put(`/dental-plans/${editingPlan._id}`, payload);
            } else {
                await api.post('/dental-plans', payload);
            }
            ...
            setFeedback({
                ...
                message: editingPlan ? 'As alterações no plano odontológico foram salvas com sucesso.' : 'O novo plano odontológico foi registrado no sistema.'
            });
        } catch (error: any) {
            setFeedback({
                type: 'error',
                title: 'Erro ao Salvar',
                message: error.response?.data?.message || 'Houve um problema ao processar o plano odontológico.',
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await api.delete(`/dental-plans/${id}`);
            fetchPlans();
            setFeedback({
                type: 'success',
                title: 'Plano Excluído',
                message: 'O plano odontológico e suas tabelas de preço foram removidos permanentemente.'
            });
        } catch (error: any) {
            setFeedback({
                type: 'error',
                title: 'Erro na Exclusão',
                message: 'Não foi possível remover o plano odontológico.',
    ...
    const handleApplyAdjustment = async () => {
        ...
        setAdjLoading(true);
        try {
            await api.post(`/dental-plans/${adjustmentModal.plan._id}/apply-adjustment`, {
    ...
    const handleApplyOperatorAdjustment = async () => {
        ...
        try {
            await api.post(`/dental-plans/adjust-by-operator`, {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center text-outfit">
                <div>
                    <h1 className="text-3xl font-black tracking-tight text-primary">Planos Odontológicos</h1>
                    <p className="text-muted-foreground font-medium">Gerencie operadoras e tabelas de preço odontológicos.</p>
                </div>
                ...
                <div className="flex gap-2">
                    ...
                    <Dialog open={open} onOpenChange={(val) => { setOpen(val); if (!val) resetForm(); }}>
                        ...
                        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto rounded-[32px] p-8 border-none font-outfit">
                            <DialogHeader>
                                <DialogTitle className="text-2xl font-black text-primary">{editingPlan ? 'Editar Plano' : 'Novo Plano Odontológico'}</DialogTitle>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <Input className="h-12 rounded-2xl bg-muted/30 border-none font-bold text-primary" value={operator} onChange={(e) => setOperator(e.target.value)} required placeholder="Ex: OdontoPrev, MetLife" />
                                    </div>
                                    <div className="space-y-2">
                                        <Label className="text-[10px] font-black uppercase text-muted-foreground ml-1">Nome do Plano</Label>
                                        <Input className="h-12 rounded-2xl bg-muted/30 border-none font-bold text-primary" value={planName} onChange={(e) => setPlanName(e.target.value)} required placeholder="Ex: Dental Max" />
                                    </div>
                                    <div className="space-y-2">
                ...
                <CardHeader className="p-8 border-b border-muted">
                    <div className="flex items-center justify-between">
                        <div className="space-y-1 font-outfit">
                            ...
                            <CardDescription className="font-medium">Lista de todos os planos odontológicos cadastrados.</CardDescription>
                        </div>
                    </div>
                </CardHeader>
                    ...
                    <div className="space-y-8">
                        {loading ? (
                            ...
                            <div className="text-center py-20 text-muted-foreground border-4 border-dashed rounded-[40px] font-outfit">
                                <Smile className="h-16 w-16 mx-auto mb-6 opacity-20" />
                            ...
                            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 font-outfit">
                                {[...plans].sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0)).map((plan: any) => (
                                    <Card key={plan._id} className="rounded-[32px] overflow-hidden border-none shadow-lg hover:shadow-2xl transition-all duration-500 bg-muted/20 group">
                                        <div className={`h-2 w-full ${plan.operator.toLowerCase().includes('odonto') ? 'bg-indigo-500' : 'bg-primary'}`} />
                                        <CardHeader className="p-6 pb-2">
                                            ...
                                            <p className="text-[10px] font-bold text-muted-foreground uppercase opacity-70">
                                                {plan.planCode || 'Sem registro ANS'}
                                            </p>
                                        </CardHeader>
